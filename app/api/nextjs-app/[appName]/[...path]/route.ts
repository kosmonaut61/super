...
export async function GET(
  request: NextRequest,
  { params }: { params: { appName: string; path: string[] } }
) {
  try {
    ...
    // Try to serve from the static directory first
    const staticPath = path.join(nextOutputPath, "static", ...filePath)
    try {
      ...
      // If static file not found, try to serve the main HTML
...
<body>
    ...
    console.error("Error serving Next.js app file:", error)
